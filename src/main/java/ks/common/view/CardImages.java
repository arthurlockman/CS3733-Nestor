...
public class CardImages {
	...
	/** Loader from which all images were derived. */
	protected CardImagesLoader cil = null;
	 ...
	 */
	public CardImages(CardImagesLoader cil) {

	}

	/**
	 * When images are created via media tracker, they must also be removed from
	 * the tracker when disposed.
	 * 
	 * @param cil   loader which had loaded up this deck.
	 * @param mt    MediaTracker from which all images are derived.
	 */
	public CardImages(CardImagesLoader cil, MediaTracker mt) {
		this.cil = cil;
		this.mt = mt;
	}
	...
	/**
	 * Disposes of all graphic elements associated with the images.
	 * <p>
	 * Properly synchronizes to avoid concurrent lookup by GUI threads. 
	 */
	public void dispose () {
		...
		for (String key : keys) {
			Image img = imageCache.remove(key);
			if (cil != null) { mt.removeImage(img); }
			img.flush();
		}
		
		// now eliminate deck from cached decks.
		cil.remove(this);

		if (backImage != null) { backImage.flush(); }
		if (cil != null) { mt.removeImage(backImage); }
		backImage = null; 
	}
}
